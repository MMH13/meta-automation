# -*- coding: utf-8 -*-
"""Suspense Ahead — reusable 9:16 reel pipeline (SVG-illustrated scenes,
Lewis narration), generalized from the first one-off reel build so daily
batches can be produced quickly. Same safety pattern as the long-form
pipeline: PID lock file, resumable per-beat state.

beats: list of dicts, each either
  {"kind": "hook"|"beat"|"end", "text": "...", "narration": "...", ...kwargs}
  {"kind": "house"|"hallway"|"door"|"window"|"clock", "caption": "...",
   "narration": "...", ...kwargs}
"""
import json
import os
...
def build(beats, out_mp4, work_dir, state_path, lock_path):
    ...
    try:
        ...
        for i, b in enumerate(beats, 1):
            ...
            fn = _RENDER[b["kind"]]
            kwargs = {k: v for k, v in b.items() if k not in ("kind", "narration")}
            if b["kind"] in ("hook", "beat", "end"):
                fn(kwargs.pop("text", ""), str(png), **kwargs)
            else:
                fn(str(png), **kwargs)
